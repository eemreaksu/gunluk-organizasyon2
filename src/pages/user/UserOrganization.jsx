...
export default function UserOrganization() {
  ...
  const deptData = deptOrder.map(dept => {
    ...
    assigned.forEach(([uid, shift]) => {
      deptHours += parseFloat(calculateNetHours(shift.shiftStart, shift.shiftEnd, shift.breakStart, shift.breakEnd));
    });
  ...
  const shareOnWhatsApp = () => {
    ...
    deptData.forEach(d => {
      if (d.users.length > 0) {
        ...
        text += `💰 Ciro: ${d.targetCiro.toLocaleString('tr-TR')} ₺\n📦 Adet: ${Math.round(d.targetAdet).toLocaleString('tr-TR')} AD\n\n`;
      }
    });
                              ...
                              <div className="space-y-1">
                                ...
                                >
                                  ...
                                  {users
                                    ...
                                    .filter(u => !dailyData?.shifts?.[u.id] && u.adSoyad)
                                    .sort((a, b) => (a.adSoyad || "").localeCompare(b.adSoyad || ""))
                                    .map(u => (
                                      <option key={u.id} value={u.id} className="text-black">{u.adSoyad}</option>
                                    ))}
                                </select>
                                <button onClick={() => setSupportModalDept(dept.name)} className="w-full bg-[#c2ff00]/10 text-[#c2ff00] border border-[#c2ff00]/20 rounded px-1 py-1 text-[8px] font-bold outline-none hover:bg-[#c2ff00]/20 uppercase tracking-widest">+ DESTEK EKLE</button>
                              </div>
                            )}
                          </div>
                        </td>
                                ...
                                <div key={uid} className="flex flex-col justify-center group h-14 border-b border-white/5 last:border-0 relative px-1">
                                  ...
                                  {!isCapturing ? (
                                    <div className="flex gap-1 mt-0.5">
                                      <select value={isAK ? 'acilis_kaptani' : isKK ? 'kapanis_kaptani' : isAA ? 'acilis_apranti' : isKA ? 'kapanis_apranti' : 'normal'} onChange={(e) => updateCaptainRole(uid, e.target.value)} className={`bg-black/30 text-[9px] ${isAK ? 'text-[#c2ff00]' : isKK ? 'text-blue-500' : isAA ? 'text-yellow-200' : isKA ? 'text-blue-300' : 'text-gray-400'} font-bold border border-white/10 rounded outline-none px-1`}>
                                      ...
                                      {dept.name === 'TRIATHLON//CRL' && (
                                        <select value={shift.task || ''} onChange={(e) => handleShiftChange(uid, 'task', e.target.value)} className="bg-black/30 text-[9px] text-[#c2ff00] border border-[#c2ff00]/20 rounded outline-none px-1 w-full">
        ...
        <div className="fixed inset-0 z-50 bg-[#1e2b6e]/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-[#1e2b6e] rounded-xl border border-white/10 w-full max-w-md shadow-2xl overflow-hidden">
            <div className="p-4 border-b border-white/10 flex justify-between items-center bg-black/20">
              <h3 className="text-[#c2ff00] font-black text-lg uppercase flex items-center gap-2"><Search className="w-5 h-5" />{supportModalDept} DESTEK EKLE</h3>
            ...
            <div className="p-4 space-y-4">
              <input type="text" placeholder="Personel Adı Ara..." value={supportSearch} onChange={e => setSupportSearch(e.target.value)} autoFocus className="w-full bg-black/30 border border-white/10 rounded-lg px-4 py-3 text-white focus:border-[#c2ff00] focus:outline-none" />
                ...
                {users
                  ...
                  .map(u => (
                    <div key={u.id} className="flex justify-between items-center bg-white/5 hover:bg-white/10 transition-colors p-3 rounded-lg border border-white/5">
                      ...
                      <button onClick={() => { addUserToDay(u.id, supportModalDept); setSupportModalDept(null); setSupportSearch(''); }} className="bg-[#c2ff00] text-[#1e2b6e] px-4 py-2 rounded-lg text-xs font-black hover:bg-[#a1d600]">EKLE</button>
                    </div>
                  ))}
              </div>
            </div>
